...
/// retried on failure, and deleted upon success.
 class PushEvent: NSObject {
    ...
    private let pushEventQueue = DispatchQueue(
        label: Constants.Queues.pushEventQueue
    )

    /// Result type to control the next step in push event processing.
    ...
    ///
    /// - Parameters:
    ///   - context: The Core Data context used for the operation. If `nil`, a default background context is used.
    ///   - objectID: The `NSManagedObjectID` of the push event to be sent.
    ///   - shouldRetry: Whether to schedule a retry on failure (default: `true`).
    ///   - completion: Called when processing finishes (success or final decision to retry).
    func sendPushEvent(
    ...
    /// Handles a single push event: decides whether to finish or retry.
    ///
    /// - Parameters:
    ///   - context: The managed object context for Core Data operations.
    ///   - objectId: The `NSManagedObjectID` of the push event to process.
    ///   - completion: Called with `.completed` when finished or `.retry` to schedule a retry.
    ...
    /// Prepares and sends a push event request, including request creation and network submission.
    ///
    /// - Parameters:
    ///   - context: The managed object context used to fetch the stored event data.
    ///   - objectID: The `NSManagedObjectID` of the stored push event entity.
    ///   - completion: Closure called with the resulting `Event` (success, error, or retry).
    ...
    /// Sends all pending `PushEventEntity` events stored in Core Data with a completion callback.
    ///
    /// This method uses a `DispatchGroup` to track completion of all send attempts.
    /// If `completion` is provided, it will be called when all sends are finished.
    ///
    /// - Parameter completion: Called when all pending events are processed (optional, default is an empty closure).
