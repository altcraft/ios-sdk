...
final class MobileEventTest: IsolatedTestCase {
    ...
    /// Creates a test MobileEventEntity with pre-filled fields
    @discardableResult
    ...
    /// Verifies PartsFactory.createMobileEventParts includes required text fields and optional JSON/text parts
    ...
    /// Verifies retryLimit increments retryCount up to max and then deletes the entity
    ...
    /// Verifies clearOldMobileEvents keeps threshold count and deletes the oldest records
    ...
    /// Verifies buildMobileEventURL appends expected query params (i/tr/t/v)
    ...
    /// Verifies createMobileEventRequest builds a valid POST multipart request with Authorization header
    ...
    /// Verifies getAllMobileEventsByTag filters by userTag and orders by time (FIFO)
    ...
    /// Verifies retryLimit returns true for an invalid (deleted) objectID
    ...
    /// Verifies RequestManager.responseProcessing maps HTTP status codes to event types correctly
