 ...
 *
 * Positive scenarios:
 *  - test_1: Resolve object returns materialized object for valid permanent ID in the same context.
 *  - test_2: Resolve object returns nil for a temporary (unsaved) object ID.
 *  - test_3: Delete entity deletes existing entity and returns true.
 *  - test_4: Delete entity returns true when entity was already deleted.
 *  - test_5: Delete entity returns true for invalid (temporary) object ID.
 *  - test_6: Retry limit increments retryCount until max, then deletes entity and returns true.
 *  - test_7: Retry limit returns true for invalid (deleted) object ID.
 */
final class CommonsDbQueriesTest: IsolatedTestCase {
    ...
    func test_2_resolveObject_returns_nil_for_temporary_id() throws {
        let temp = MobileEventEntity(context: viewContext)
    ...
    /// test_6: Retry limit increments retryCount until max, then deletes and returns true
